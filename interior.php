...
<h1 style="text-align:center">RAD Clothing: Interiors</h1>
...
<h1>Interiors</h1>
<p>Here is a list of all of the avaliable interiors in our store.</p>
...
<form action="interior.php" method="post">
    ...
    <p>
        <label for="interiorType">Insert Interior Type:</label>
        <input type="text" name="interiorType" id="interiorType">
    </p>
    <p>
...
<li><a href = 'updatePriceI.php'>Update Price</a></li>
<li><a href = 'updateQuantityI.php'>Update Quantity</a><l/li>
</body>
</html>
...
$interiorType = mysqli_real_escape_string($link, $_REQUEST['interiorType']);
...
$sql = "INSERT INTO RADClothing.Interior (productNumber, InteriorType, size, color, price, quantity) VALUES ('$productNumber', '$interiorType', '$size', '$color', '$price', '$quantity')";
if(mysqli_query($link, $sql)){
    echo "Records added successfully.";
} else{
    echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
...
$sql = "SELECT * FROM RADClothing.Interior";
...
while($row = $result->fetch_assoc()) {
	echo "<tr>";
		...
		echo "<td>".$row["InteriorType"]. "</td>";
		echo "<td>".$row["size"]. "</td>";
		echo "<td>".$row["color"]. "</td>";
		echo "<td>".$row["price"]. "</td>";
		echo"<td>".$row["quantity"]."</td>";
		echo "<td><a href='deleteI.php?pNum=".$row["productNumber"]."'>Delete</a></td>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
